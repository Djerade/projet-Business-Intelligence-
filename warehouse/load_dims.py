...
class DimensionLoader:
    ...
    def load_countries(self, countries_df: pd.DataFrame) -> None:
        """
        Load countries into dim_country.
        ...
        dim_cols = ['country_code', 'country_name', 'iso2_code', 'iso3_code',
                   'region_code', 'income_level', 'currency_code', 'created_at', 'updated_at']
    ...
    def load_regions(self, regions_df: pd.DataFrame) -> None:
        ...
        regions_df['created_at'] = pd.Timestamp.now()
        
        # Ensure all columns exist
        if 'subregion_name' not in regions_df.columns:
            regions_df['subregion_name'] = None
        
        # Load to database
        regions_df[['region_code', 'region_name', 'subregion_name', 'created_at']].to_sql(
            'dim_region',
            self.engine,
            if_exists='append',
            index=False
        ...
        logger.info(f"Loaded {len(regions_df)} regions into dim_region")
